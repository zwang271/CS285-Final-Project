...
class MaxiMinPolicy(object):
    ...
    def __init__(self, max_search_depth=1, use_ab=True):
        self.env = None
    ...
    def search_ab(self, env, depth, perspective, alpha, beta):
        ...
        if (
            ...
            return perspective * env.eval(), None
        else:
            ...
            for move in possible_moves:
                ...
                value = max(
                    -self.search_ab(new_env, depth + 1, -perspective, -beta, -alpha)[0],
                    alpha
                    )
            ...
            new_env.close()
            ix = int(np.argmax(value_list))
            return value_list[ix], possible_moves[ix]
    ...
    def get_action(self, obs):
        ...
        if self.use_ab:
            disk_cnt, move = self.search_ab(env=self.env,
                                        depth=0,
